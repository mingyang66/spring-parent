...
<dependency>
  ...
  <artifactId>emily-logger</artifactId>
...
- 新建emily-logger sdk，不依赖任何三方组件，不依赖任何web容器；
- 新建emily-spring-boot-logger sdk，为基于springboot的项目提供开箱即用的starter;

##### 二、emily-logger基础库SDK支持哪些功能
...
- 完全基于emily-logger sdk组件为springboot提供的starter组件；
...
- emily-logger组件SDK使用方法，调用初始化方法后即可开始日志记录的愉快旅程
