 ...
 * @since : 2022/01/10
 */
public class LogbackTimeBasedRollingPolicy implements LogbackRollingPolicy {
    private final LoggerContext context;
    private final LogbackProperties properties;

    public LogbackTimeBasedRollingPolicy(LoggerContext context, LogbackProperties properties) {
    ...
    public RollingPolicy getRollingPolicy(RollingFileAppender<ILoggingEvent> appender, String loggerPath) {
        LogbackProperties.TimeRollingPolicy rollingPolicy = properties.getAppender().getTimeRollingPolicy();
        //文件归档大小和时间设置
