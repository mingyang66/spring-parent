...
public class Base64Utils {
    ...
    public static String encoder(String encoderStr, String charset) {
        try {
            if (StringUtils.isEmpty(encoderStr)) {
                throw new SystemException(AppHttpStatus.NULL_POINTER_EXCEPTION.getStatus(), "编码字符串不可以为空");
            }
            ...
            throw new SystemException(AppHttpStatus.ILLEGAL_ARGUMENT_EXCEPTION);
        }
    }
    ...
    public static String decoder(String decoderStr, String charset) {
        try {
            if (StringUtils.isEmpty(decoderStr)) {
                throw new SystemException(AppHttpStatus.NULL_POINTER_EXCEPTION.getStatus(), "解码字符串不可以为空");
            }
            if (StringUtils.isEmpty(charset)) {
                charset = CharsetUtils.UTF_8;
            }
    ...
    public static void encoderWrap(String filePath, String encoderStr, String charset) {
        try {
            ...
            throw new SystemException(AppHttpStatus.ILLEGAL_ARGUMENT_EXCEPTION.getStatus(), StringUtils.join("指定路径", filePath, "的文件不存在," + e));
        } catch (UnsupportedEncodingException e) {
            throw new SystemException(AppHttpStatus.ILLEGAL_ARGUMENT_EXCEPTION);
        }
    }
