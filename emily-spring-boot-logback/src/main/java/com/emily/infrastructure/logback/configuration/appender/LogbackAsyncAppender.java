...
public class LogbackAsyncAppender {
    ...
    public AsyncAppender getAsyncAppender(Appender ref) {
        ...
        //队列的最大容量，默认为 256
        appender.setQueueSize(properties.getAsync().getAsyncQueueSize());
        //默认，当队列还剩余 20% 的容量时，会丢弃级别为 TRACE, DEBUG 与 INFO 的日志，仅仅只保留 WARN 与 ERROR 级别的日志。想要保留所有的事件，可以设置为 0
        appender.setDiscardingThreshold(properties.getAsync().getAsyncDiscardingThreshold());
        ...
        appender.setMaxFlushTime(properties.getAsync().getAsyncMaxFlushTime());
        //默认为 false，在队列满的时候 appender 会阻塞而不是丢弃信息。设置为 true，appender 不会阻塞你的应用而会将消息丢弃
        appender.setNeverBlock(properties.getAsync().isAsyncNeverBlock());
