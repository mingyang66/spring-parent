...
public class MoniterMQTask implements Runnable {
    private static final Logger logger = LoggerFactory.getLogger(MoniterMQTask.class);
        ...
        while (true) {
            ...
            switch (type) {
                ...
                case ZMQ.EVENT_ACCEPT_FAILED:
                    logger.info("监控地址：" + event.getAddress() + "--监控事件：" + "无法接受客户端的连接" + "--值：" + event.getValue());
                    break;
                /**
                 * 连接关闭，当连接的底层描述符已经关闭时，将会触发EVENT_CLOSED事件
                 */
                case ZMQ.EVENT_CLOSED:
                    logger.info("监控地址：" + event.getAddress() + "--监控事件：" + "连接关闭" + "--值：" + event.getValue());
                    break;
                /**
                ...
                case ZMQ.EVENT_DISCONNECTED:
                    logger.info("监控地址：" + event.getAddress() + "--监控事件：" + "中断会话" + "--值：" + event.getValue());
                    break;
                /**
                 * SOCKET监视器停止，当SOCKET套接字的监视器停止时，将会触发EVENT_MONITOR_STOPPED事件
                 */
                case ZMQ.EVENT_MONITOR_STOPPED:
                    logger.info("监控地址：" + event.getAddress() + "--监控事件：" + "SOCKET监视器停止" + "--值：" + event.getValue());
                    break;
                /**
                 * 已成功协商协议，当流引擎（TCP/IPC）成功与对等方协商协议时，将会触发EVENT_HANDSHAKE_PROTOCOL事件
                 */
                case ZMQ.EVENT_HANDSHAKE_PROTOCOL:
                    logger.info("监控地址：" + event.getAddress() + "--监控事件：" + "已成功协商协议" + "--值：" + event.getValue());
                    break;
                default:
                    break;
            }
