...
public class MoniterMQTask implements Runnable{
    ...
    @Override
    public void run() {
        LogbackFactory.info(MoniterMQTask.class, "启动监控ZEROMQ事件线程...");
        ...
        while (true) {
            ...
            switch (type){
                ...
                case ZMQ.EVENT_CONNECTED:
                    LogbackFactory.info(MoniterMQTask.class, "监控地址："+event.getAddress()+"--监控事件："+"连接已建立"+"--值："+event.getValue());
                    break;
                /**
                 * 同步连接失败，正在轮询，当立即连接尝试被延迟并且正在轮询其完成时，将会出发EVENT_CONNECT_DELAYED事件
                 */
                case ZMQ.EVENT_CONNECT_DELAYED:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"同步连接失败，正在轮询"+"--值："+event.getValue());
                    break;
                /**
                 * 异步连接或重连尝试，当重新连接计时器正在处理连接尝试时，EVENT_CONNECT_RETRIED 事件将会被出发，重新计算每次重连的时间间隔
                 */
                case ZMQ.EVENT_CONNECT_RETRIED:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"异步连接或重连尝试"+"--值："+event.getValue());
                    break;
                /**
                 * SOCKET绑定到地址，准备接收连接，当套接字成功绑定到端口上时，将触发EVENT_LISTENING 事件，值是新绑定套接字的FD
                 */
                case ZMQ.EVENT_LISTENING:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"SOCKET绑定到地址，准备接收连接"+"--值："+event.getValue());
                    break;
                ...
                case ZMQ.EVENT_ACCEPT_FAILED:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"无法接受客户端的连接"+"--值："+event.getValue());
                    break;
                /**
                 * 连接关闭，当连接的底层描述符已经关闭时，将会触发EVENT_CLOSED事件
                 */
                case ZMQ.EVENT_CLOSED:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"连接关闭"+"--值："+event.getValue());
                    break;
                /**
                ...
                case ZMQ.EVENT_DISCONNECTED:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"中断会话"+"--值："+event.getValue());
                    break;
                /**
                 * SOCKET监视器停止，当SOCKET套接字的监视器停止时，将会触发EVENT_MONITOR_STOPPED事件
                 */
                case ZMQ.EVENT_MONITOR_STOPPED:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"SOCKET监视器停止"+"--值："+event.getValue());
                    break;
                /**
                 * 已成功协商协议，当流引擎（TCP/IPC）成功与对等方协商协议时，将会触发EVENT_HANDSHAKE_PROTOCOL事件
                 */
                case ZMQ.EVENT_HANDSHAKE_PROTOCOL:
                    LogbackFactory.info(MoniterMQTask.class,"监控地址："+event.getAddress()+"--监控事件："+"已成功协商协议"+"--值："+event.getValue());
                    break;
                default:
                    break;
            }
